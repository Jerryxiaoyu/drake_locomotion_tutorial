...
laikago = Laikago(laikago_urdf_path)
...
laikago_model = plant_parser.AddModelFromFile(file_name=laikago_urdf_path,
                              model_name="laikago")

ground_model = plant_parser.AddModelFromFile(file_name=ground_urdf_path, model_name='ground')
plant.WeldFrames(plant.world_frame(),
                 plant.GetFrameByName("link", ground_model))
...
#                                 np.ones((plant.get_input_port(3).size(), 1))*torque))
# builder.Connect(torque_system.get_output_port(0),
#                 plant.get_input_port(3))
...
builder.Connect(plant.get_output_port(2),
...
torque = 1.0
torque_system = builder.AddSystem(ConstantVectorSource(
                                np.ones((plant.get_input_port(4).size(), 1))*torque))
builder.Connect(torque_system.get_output_port(0),
                plant.get_input_port(4))
