...
ground_model = mbp_parser.AddModelFromFile(
    file_name=ground_urdf_path, model_name='ground')
...
if is_fixed:
    ...
    X_Wrobot.set_translation(np.array([0, 0, 1.2]))
    mbp.AddJoint(WeldJoint(name='weld_ground_to_world',
                  parent_frame_P=mbp.world_body().body_frame(),
                  child_frame_C= mbp.GetBodyByName("pelvis", atlas_model).body_frame(),
                  X_PC=X_Wrobot))


# Add gravity
mbp.AddForceElement(UniformGravityFieldElement([0, 0, 90.81]))
...
mbp_context.FixInputPort(
    ...
    np.ones(mbp.get_input_port(4).size())*10  )
